...
import { 
  ShoppingCart, ChevronRight, ChevronLeft, CheckCircle, MapPin, Phone,
  Search, SlidersHorizontal, Star, Clock, Flame, Utensils, X, Info, HelpCircle
} from 'lucide-react';
...
const getMealTimeConfig = (hour) => {
  if (hour >= 5 && hour < 11) {
    ...
    return {
      ...
      defaultCategory: 'Mains',
      special: {
        id: 'special-1', // Custom Chef's Butter Chicken
...
export const GuestApp = ({ menuItems, orders, setOrders, socketConnected, config = CONFIG }) => {
  ...
  // Custom Pricing helpers for item in cart
  const getItemUnitPrice = (item) => {
    let price = item.price;
    if (selectedCustomizations[`${item.id}-opt-portions`]) {
      price += Math.round(item.price * 0.4);
    }
    if (selectedCustomizations[`${item.id}-opt-extra`]) {
      price += 40;
    }
    return price;
  };
  ...
  const placeOrder = () => {
    ...
    const locationString = serviceType === 'ROOM' ? roomNumber : `Table ${tableNumber}`;
  ...
  const trackingOrder = orders.find(o => o.id === lastOrderId) || orders.find(o => o.room === (serviceType === 'ROOM' ? roomNumber : `Table ${tableNumber}`));
  ...
  return (
    <div style={{ 
      ...
      {currentScreen === 'menu' && (
        <div className="animate-fade-in">
          ...
          <div style={{ position: 'sticky', top: 0, zIndex: 100, background: C.white, padding: '18px 20px', boxShadow: '0 6px 24px rgba(6,6,6,0.03)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                {/* Location Display Box (Locked/Non-editable for QR code security) */}
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: C.brass, fontSize: 13, marginTop: 5, background: `${C.brass}12`, padding: '4px 10px', borderRadius: 8 }}>
                  ...
                  <span style={{ fontWeight: 750 }}>
                    {serviceType === 'ROOM' ? `Room ${roomNumber}` : `Table ${tableNumber}`}
                  </span>
                </div>
              </div>
            ...
            <div style={{ marginTop: 16, display: 'flex', gap: 10, alignItems: 'center' }}>
              ...
              <button 
                onClick={() => {
                  setVegOnly(!vegOnly);
                  showToast(vegOnly ? "Showing all items" : "Filtered: Vegetarian only 🥬");
                }}
                style={{ 
                  background: vegOnly ? `${C.brass}20` : C.borderLight, 
                  border: `1.5px solid ${vegOnly ? C.brass : 'transparent'}`, 
                  borderRadius: 12, 
                  padding: 12, 
                  cursor: 'pointer', 
                  display: 'flex', 
                ...
                return (
                  <button 
                    ...
                    style={{
                      ...
                      fontWeight: 700,
                      border: 'none',
                      background: isSelected ? C.brass : C.borderLight,
                      color: isSelected ? C.white : C.textSub,
                      boxShadow: isSelected ? `0 6px 16px ${C.brass}30` : 'none',
                      display: 'flex',
                      alignItems: 'center',
                      gap: 6,
                      cursor: 'pointer',
          ...
          <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 28 }}>
            ...
            <div style={{
              background: `linear-gradient(135deg, ${C.brass} 0%, #C2801E 100%)`,
              borderRadius: 24,
              padding: '20px 24px',
              color: C.white,
              position: 'relative',
              overflow: 'hidden',
              boxShadow: '0 10px 25px rgba(184, 139, 58, 0.15)'
            }}>
              <div style={{
                position: 'absolute',
                top: '-40px',
              ...
              <div style={{ width: '60%', position: 'relative', zIndex: 1 }}>
                ...
                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, lineHeight: 1.2, color: C.white }}>
                  {mealConfig.special.name}
                </h3>
                <p style={{ margin: '6px 0 16px 0', fontSize: 12, opacity: 0.9, lineHeight: 1.3 }}>
                  Limited time recipe crafted by our head chef.
                </p>
                                ...
                                {qty === 0 ? (
                                  <button 
                                    ...
                                    style={{ 
                                      ...
                                      fontSize: 16,
                                      fontWeight: 'bold',
                                      boxShadow: `0 4px 10px ${C.brass}30`
                                    }}
                                  >
                                    +
                                  </button>
          ...
          {cart.length === 0 ? (
            <div style={{ textAlign: 'center', marginTop: 60, padding: 20 }}>
              ...
              <button 
                ...
                style={{ background: C.brass, color: C.white, border: 'none', padding: '14px 28px', borderRadius: 16, fontWeight: 700, fontSize: 14, cursor: 'pointer', boxShadow: `0 6px 20px ${C.brass}30`, fontFamily: 'inherit' }}
              >
                Browse Menu
              </button>
            </div>
            ...
            <>
                    ...
                    <Card key={item.id} style={{ padding: 16, border: 'none', borderRadius: 20, boxShadow: '0 4px 16px rgba(15,27,43,0.03)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                          ...
                          {itemCustomizations.length > 0 && (
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
                              {itemCustomizations.map(c => (
                                <span key={c} style={{ fontSize: 10, background: C.borderLight, color: C.textSub, padding: '2px 6px', borderRadius: 6, fontWeight: 600 }}>
                                  {c}
                                </span>
                              ))}
                            </div>
                          ...
                          {itemInstructions && (
                            <p style={{ margin: '6px 0 0 0', fontSize: 12, color: C.brass, fontStyle: 'italic' }}>
                              "{itemInstructions}"
                            </p>
                          )}
                          
                          <div style={{ color: C.textSub, fontSize: 13, marginTop: 8, fontWeight: 600 }}>
                            ₹{unitPrice} each
                          </div>
                        </div>
              ...
              {/* Service Details Card (Non-editable for safety) */}
              <Card style={{ padding: 16, border: 'none', borderRadius: 20, marginBottom: 20, boxShadow: '0 4px 16px rgba(15,27,43,0.03)' }}>
                <h3 style={{ fontSize: 14, fontWeight: 800, color: C.text, margin: '0 0 12px 0' }}>Service Location</h3>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: C.borderLight, padding: 12, borderRadius: 12 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>
                      {serviceType === 'ROOM' ? 'In-room Dining' : 'Restaurant Table Service'}
                    </div>
                    <div style={{ fontSize: 12, color: C.textSub }}>
                      {serviceType === 'ROOM' ? `Delivered to Room ${roomNumber}` : `Served at Table ${tableNumber}`}
                    </div>
                  </div>
                </div>
              </Card>
              ...
              <Card style={{ padding: 16, border: 'none', borderRadius: 20, marginBottom: 20, boxShadow: '0 4px 16px rgba(15,27,43,0.03)' }}>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                  <button onClick={() => setDeliveryPreference('ALL_AT_ONCE')} style={{ flex: 1, padding: 12, borderRadius: 12, border: `1.5px solid ${deliveryPreference === 'ALL_AT_ONCE' ? C.brass : C.border}`, background: deliveryPreference === 'ALL_AT_ONCE' ? `${C.brass}10` : C.white, color: deliveryPreference === 'ALL_AT_ONCE' ? C.brass : C.textSub, fontWeight: 700, fontSize: 13, cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit' }}>
                    All at once
                  </button>
                  <button onClick={() => setDeliveryPreference('AS_READY')} style={{ flex: 1, padding: 12, borderRadius: 12, border: `1.5px solid ${deliveryPreference === 'AS_READY' ? C.brass : C.border}`, background: deliveryPreference === 'AS_READY' ? `${C.brass}10` : C.white, color: deliveryPreference === 'AS_READY' ? C.brass : C.textSub, fontWeight: 700, fontSize: 13, cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit' }}>
                    As soon as ready
                  </button>
                </div>
                <textarea 
              ...
              <button 
                ...
                style={{ 
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  boxShadow: `0 8px 24px ${C.brass}40`, 
                  display: 'flex', 
                  justifyContent: 'center', 
                  alignItems: 'center', 
                  gap: 8,
        ...
        <div className="animate-fade-in" style={{ padding: 24, textAlign: 'center' }}>
          {trackingOrder ? (
            <>
              ...
              {/* Token Details */}
              <div style={{ 
                background: `linear-gradient(135deg, ${C.emerald} 0%, #152E4B 100%)`, 
                borderRadius: 24, 
                padding: '24px 20px', 
                marginTop: 20, 
                marginBottom: 24, 
                color: C.white, 
                boxShadow: '0 12px 32px rgba(7, 20, 40, 0.15)' 
              }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 15, fontWeight: 650 }}>
                  <MapPin size={16} color={C.brass} /> {trackingOrder.room}
                </div>
              </div>
                    ...
                    return (
                      <div key={step} style={{ display: 'flex', alignItems: 'center', gap: 16, zIndex: 1, position: 'relative' }}>
                        <div style={{ 
                          ...
                          height: 24, 
                          borderRadius: '50%', 
                          background: isPast ? C.brass : isActive ? C.brass : C.white, 
                          border: `2px solid ${isPast ? C.brass : isActive ? C.brass : C.border}`, 
                          display: 'flex', 
                          alignItems: 'center', 
                          justifyContent: 'center' 
                        }}>
                        ...
                        <span style={{ 
                          ...
                          color: isActive ? C.text : isPast ? C.textSub : C.textMuted, 
                          fontSize: 14 
                        }}>
                          {labels[step]}
                        </span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {trackingOrder.items.map((item, idx) => (
                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 14 }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 700, color: C.brass }}>
                        ₹{item.price * item.qty}
                      </span>
                    </div>
                  ))}
              ...
              <button 
                onClick={() => setCurrentScreen('menu')} 
                style={{ background: 'none', border: 'none', color: C.brass, fontWeight: 700, cursor: 'pointer', fontSize: 14, fontFamily: 'inherit' }}
              >
                ← Back to Menu
              </button>
            </>
      ...
      {currentScreen === 'support' && (
        <div className="animate-fade-in" style={{ padding: 24 }}>
          <h2 className="serif" style={{ margin: '0 0 8px 0', fontSize: 24, fontWeight: 800, color: C.text }}>
            {serviceType === 'ROOM' ? 'Hotel Directory' : 'Table Service Support'}
          </h2>
          <p style={{ color: C.textSub, fontSize: 14, margin: '0 0 24px 0' }}>
            {serviceType === 'ROOM' ? 'Need assistance during your stay? We are here to help.' : 'Request help directly from your table.'}
          </p>
            ...
            {serviceType === 'ROOM' ? (
              <>
                <Card style={{ padding: 16, border: 'none', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 4px 16px rgba(15,27,43,0.03)' }}>
                  <div style={{ background: `${C.brass}15`, padding: 12, borderRadius: '50%' }}>
                    <Phone size={20} color={C.brass} />
                  </div>
                  <div style={{ flex: 1 }}>
                ...
                <Card style={{ padding: 16, border: 'none', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 4px 16px rgba(15,27,43,0.03)' }}>
                  ...
                  <button 
                    onClick={() => showToast(`Housekeeping has been requested for Room ${roomNumber}!`)} 
                    style={{ background: C.brass, color: C.white, border: 'none', padding: '8px 16px', borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
                  >
                    Request
                  </button>
                </Card>
              ...
              <>
                <Card style={{ padding: 16, border: 'none', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 4px 16px rgba(15,27,43,0.03)' }}>
                  <div style={{ background: `${C.brass}15`, padding: 12, borderRadius: '50%' }}>
                    <Utensils size={20} color={C.brass} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 800, fontSize: 15, color: C.text }}>Call a Waiter</div>
                    <div style={{ color: C.textSub, fontSize: 13 }}>Request immediate waiter service</div>
                  </div>
                  <button 
                    onClick={() => showToast(`Waiter requested for Table ${tableNumber}! Service is on the way.`)} 
                    style={{ background: C.brass, color: C.white, border: 'none', padding: '8px 16px', borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
                  >
                    Call Waiter
                  </button>
                ...
                <Card style={{ padding: 16, border: 'none', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 4px 16px rgba(15,27,43,0.03)' }}>
                  <div style={{ background: `${C.brass}15`, padding: 12, borderRadius: '50%' }}>
                    <HelpCircle size={20} color={C.brass} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 800, fontSize: 15, color: C.text }}>Request Manager</div>
                    <div style={{ color: C.textSub, fontSize: 13 }}>Escalate help or payment request</div>
                  </div>
                  <button 
                    onClick={() => showToast(`Manager has been notified for Table ${tableNumber}.`)} 
                    style={{ background: C.brass, color: C.white, border: 'none', padding: '8px 16px', borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
                  >
                    Call Manager
                  </button>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              if (serviceType === 'TABLE' && (info.title.includes('Gym') || info.title.includes('Check-out'))) return null;
              return (
                <Card 
                  key={idx} 
                  onClick={() => {
                    ...
                    } else if (info.title.includes('Check-out')) {
                      showToast('Reception notified for Express Check-out! Please review your folio.');
                    } else {
      ...
      <div style={{
        ...
        borderRadius: 24,
        padding: '10px 14px',
        boxShadow: '0 10px 30px rgba(15, 27, 43, 0.08)',
        display: 'flex',
          ...
          return (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                position: 'relative',
                color: isActive ? C.brass : C.textSub,
                transition: 'all 0.2s',
                fontFamily: 'inherit'
              }}
            >
              <div style={{
                ...
                left: 0,
                right: 0,
                background: isActive ? `${C.brass}12` : 'transparent',
                borderRadius: 16,
          ...
          <div className="animate-slide-up" style={{
            ...
            <div className="hide-scrollbar" style={{ padding: 24, overflowY: 'auto', flex: 1 }}>
                      ...
                      <label key={opt.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', borderRadius: 12, background: isChecked ? `${C.brass}08` : C.borderLight, border: `1.5px solid ${isChecked ? C.brass : 'transparent'}`, cursor: 'pointer', transition: 'all 0.15s' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <input 
                            ...
                            style={{ accentColor: C.brass, width: 16, height: 16 }}
                          />
              ...
              <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 20, marginBottom: 12 }}>
                ...
                <textarea 
                  placeholder="Allergies, spice preferences, or room details..."
            ...
            <div style={{ padding: '16px 24px', borderTop: `1px solid ${C.border}`, background: C.white, display: 'flex', gap: 16, alignItems: 'center', flexShrink: 0 }}>
              {(() => {
                ...
                return (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, border: `1.5px solid ${C.brass}`, borderRadius: 12, padding: '8px 16px', background: C.white }}>
                    <button 
                      onClick={() => {
                        if (qty > 0) updateQty(selectedItem, -1);
                      }} 
                      style={{ border: 'none', background: 'none', color: C.brass, fontWeight: 800, fontSize: 20, cursor: 'pointer', padding: '0 4px' }}
                    >
                      −
                    </button>
                    <span style={{ fontWeight: 800, color: C.text, fontSize: 16, minWidth: 20, textAlign: 'center' }}>
                      {qty}
                    </span>
                    <button 
                      onClick={() => updateQty(selectedItem, 1)} 
                      style={{ border: 'none', background: 'none', color: C.brass, fontWeight: 800, fontSize: 20, cursor: 'pointer', padding: '0 4px' }}
                    >
                      +
                    </button>
              ...
              {(() => {
                ...
                const extraCost = [
                  ...
                  return acc + (selectedCustomizations[`${selectedItem.id}-${opt.id}`] ? opt.price : 0);
                }, 0);
                ...
                return (
                  <button 
                    ...
                    style={{
                      ...
                      fontSize: 15,
                      cursor: 'pointer',
                      boxShadow: `0 6px 20px ${C.brass}30`,
                      textAlign: 'center',
                      fontFamily: 'inherit'
                    }}
                  >
